"""Utilidades comunes a los gráficos del TP2 (prefijo p2_)."""
import sys
...
def miles(v, dec=0):
    """Número con punto de miles y coma decimal: 1234.5 -> '1.234,5'."""
    s = f"{v:,.{dec}f}"
    return s.replace(",", "X").replace(".", ",").replace("X", ".")


def fmt_miles(dec=0):
    return plt.FuncFormatter(lambda v, p: miles(v, dec))
...
def flecha(ax, x0, y0, x1, y1, color=None, lw=1.3, texto=None, tdx=0, tdy=0.12, size=8.5, tcolor=None,
    ...
    if texto:
        ax.text((x0 + x1) / 2 + tdx, (y0 + y1) / 2 + tdy, texto, ha="center", va="bottom", fontsize=size,
