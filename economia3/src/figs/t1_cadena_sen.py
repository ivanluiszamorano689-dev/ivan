...
fig, ax, H = canvas(6.9, 3.8)

# marco de la libertad (supracapacidad)
rbox(ax, 0.6, 12.6, 98.8, H - 13.2, "violet", fill="#fbfaff", lw=1.3, r=2.5, ls=(0, (5, 3)), z=1)
txt(ax, 2.6, H - 3.0, "LIBERTAD (supracapacidad)", size=8.2, color=DARK["violet"], weight="bold", ha="left")
txt(ax, 31.5, H - 3.0, "el marco que hace posible elegir (p. ej., el Estado garantiza el derecho a estudiar)",
...
for i, (col, t1, t2, body, tag, ej) in enumerate(boxes):
    ...
    pill(ax, x + bw / 2, yb + 7.7, tag, color=col, size=7.3, h=3.1, fill="solid", tcolor="white")
    txt(ax, x + bw / 2, yb + 3.1, ej, size=6.9, color=C["ink2"], style="italic", ls=1.2)
...
yq = 5.6
txt(ax, 25.0, yq + 4.7, "¿Dónde mira cada enfoque?", size=7.6, color=C["ink2"], weight="bold")
